...
function [X, errList] = ADMMLRTC(T, Omega, alpha, beta, maxIter, epsilon, X)
...
for k = 1: maxIter
    if mod(k, 20) == 0
        fprintf('scLRTC: iterations = %d   difference=%f\n', k, errList(k-1));
    end
    beta = beta * 1.05;
...
errList = errList(1:k);
fprintf('scLRTC ends: total iterations = %d   difference=%f\n\n', k, errList(k));
